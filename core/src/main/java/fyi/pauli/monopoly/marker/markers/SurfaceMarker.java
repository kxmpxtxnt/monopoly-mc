  ...
  public void mark() {
    ...
    for (int i = 0; i < this.surfaceSides.length; i++) {
      switch (this.surfaceSides[i]) {
        case NEG_Z ->
            super.spawnMarker(loc1, display -> display.setTransformation(super.scale(new Vector3f((float) this.area.getWidthX(), (float) this.area.getHeight(), 0))));
        case NEG_X ->
            super.spawnMarker(loc1, display -> display.setTransformation(super.scale(new Vector3f(0, (float) this.area.getHeight(), (float) this.area.getWidthZ()))));
        case BOTTOM ->
            super.spawnMarker(loc1, display -> display.setTransformation(super.scale(new Vector3f((float) this.area.getWidthX(), 0, (float) this.area.getWidthZ()))));
        case POS_Z ->
            super.spawnMarker(loc2, display -> display.setTransformation(super.scale(new Vector3f((float) -this.area.getWidthX(), (float) -this.area.getHeight(), 0))));
        case POS_X ->
            super.spawnMarker(loc2, display -> display.setTransformation(super.scale(new Vector3f(0, (float) -this.area.getHeight(), (float) -this.area.getWidthZ()))));
        case TOP ->
            super.spawnMarker(loc2, display -> display.setTransformation(super.scale(new Vector3f((float) -this.area.getWidthX(), 0, (float) -this.area.getWidthZ()))));
      }
    }
  }
